...
Implements tabular Q-learning for the relationship dynamics environment.
...
class QLearningAgent:
    """
    Q-Learning agent for relationship dynamics simulator.

    Uses tabular Q-learning with discretized state space.
    States are discretized into bins for emotion, trust, and conflict.
    """
    ...
    def update(
        ...
        # Next state Q-value (max over actions)
        if done:
            next_q = 0.0
        else:
            next_q_values = self.q_table[discrete_next_state].copy()
            # Apply personality bias to next state Q-values
            for action_idx in range(self.num_actions):
                next_q_values[action_idx] += self.personality.get_action_bias(
                    action_idx
                )
